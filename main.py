...
def install_docker():
    with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}")) as progress:
        ...
        try:
            ...
            subprocess.run(["echo", "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.asc] https://download.docker.com/linux/ubuntu $(. /etc/os-release && echo $VERSION_CODENAME) stable", "|", "sudo", "tee", "/etc/apt/sources.list.d/docker.list"], shell=True, check=True)
            subprocess.run(["sudo", "apt-get", "update"], check=True)
