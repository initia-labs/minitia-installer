...
parser.add_argument(
    "-c", "--cosmovisor", action="store_true", help="Install cosmovisor"
)
...
"""
    ...
    print(
        "🧪 Try running: "
        + bcolors.OKGREEN
        + f"osmosisd status --home {osmosis_home}"
        + bcolors.ENDC
    )
    print()
...
"""
    ...
    if using_service:
        ...
        if using_cosmovisor:
            ...
            print(
                bcolors.OKGREEN
                + f"DAEMON_NAME=osmosisd DAEMON_HOME={osmosis_home} cosmovisor run start"
                + bcolors.ENDC
            )
        else:
            print("🧪 To start osmosisd run: ")
            print(
                bcolors.OKGREEN + f"osmosisd start --home {osmosis_home}" + bcolors.ENDC
            )
...
def select_install():
    ...
    if args.install:
        if args.install == "node":
            ...
            print(
                bcolors.RED
                + f"Invalid setup {args.install}. Please choose a valid setup.\n"
                + bcolors.ENDC
            )
            sys.exit(1)
...
💡 You can select the installation using the --install flag.
        ...
        while True:
            ...
            if choice not in [
                InstallChoice.NODE,
                InstallChoice.CLIENT,
                InstallChoice.LOCALOSMOSIS,
            ]:
...
💡 You can overwrite the osmosis home using --overwrite flag.
    ...
    try:
        subprocess.run(
            ["rm", "-rf", osmosis_home], stderr=subprocess.DEVNULL, check=True
        )
...
def select_pruning(osmosis_home):
    ...
    if args.pruning:
        if args.pruning == "default":
            ...
            print(
                bcolors.RED
                + f"Invalid pruning setting {args.pruning}. Please choose a valid setting.\n"
                + bcolors.ENDC
            )
            sys.exit(1)
...
💡 You can select the pruning settings using the --pruning flag.
    """
        ...
        while True:
            ...
            if choice not in [
                PruningChoice.DEFAULT,
                PruningChoice.NOTHING,
                PruningChoice.EVERYTHING,
            ]:
                print("Invalid input. Please choose a valid option.")
            else:
                break
    ...
    if choice == PruningChoice.DEFAULT:
        ...
        subprocess.run(
            ['sed -i -E \'s/pruning = "default"/pruning = "nothing"/g\' ' + app_toml],
            shell=True,
        )

    elif choice == PruningChoice.EVERYTHING:
        primeNum = random.choice(
            [x for x in range(11, 97) if not [t for t in range(2, x) if not x % t]]
        )
        subprocess.run(
            ['sed -i -E \'s/pruning = "default"/pruning = "custom"/g\' ' + app_toml],
            shell=True,
        )
        subprocess.run(
            [
                'sed -i -E \'s/pruning-keep-recent = "0"/pruning-keep-recent = "10000"/g\' '
                + app_toml
            ],
            shell=True,
        )
        subprocess.run(
            [
                'sed -i -E \'s/pruning-interval = "0"/pruning-interval = "'
                + str(primeNum)
                + "\"/g' "
                + app_toml
            ],
        ...
        print(
            bcolors.RED
            + f"Invalid pruning setting {choice}. Please choose a valid setting.\n"
            + bcolors.ENDC
        )
        sys.exit(1)
...
def customize_config(home, network):
    ...
    if network == NetworkChoice.TESTNET:
        ...
        subprocess.run(
            [
                'sed -i -E \'s/persistent_peers = ""/persistent_peers = "'
                + peers
                + "\"/g' "
                + config_toml
            ],
    ...
    # osmosis-1 configuration
        ...
        print(
            bcolors.RED
            + f"Invalid network {network}. Please choose a valid setting.\n"
            + bcolors.ENDC
        )
        sys.exit(1)
...
def download_binary(network):
    ...
    if not args.overwrite:
        ...
        try:
            ...
            print(
                "osmosisd is already installed at "
                + bcolors.OKGREEN
                + f"{binary_path}"
                + bcolors.ENDC
            )
            while True:
                choice = (
                    input(
                        "Do you want to skip the download or overwrite the binary? (skip/overwrite): "
                    )
                    .strip()
                    .lower()
                )
    ...
    if (
        operating_system in binary_urls
        and architecture in binary_urls[operating_system]
    ):
        binary_url = binary_urls[operating_system][architecture]
    else:
        print(
            f"Binary download URL not available for {operating_system}/{architecture}"
        )
        sys.exit(0)
...
def download_addrbook(network, osmosis_home):
    ...
    if addrbook_url:
        try:
            ...
            subprocess.run(
                ["wget", addrbook_url, "-q", "-O", addrbook_path], check=True
            )
...
Choose one of the following snapshots:
        ...
        for idx, snapshot in enumerate(snapshot_info):
            ...
            wrapped_data = [
                textwrap.fill(data, width=width)
                for data, width in zip(row_data, column_widths)
            ]
            formatted_row = " | ".join(
                f"{data:{width}}" for data, width in zip(wrapped_data, column_widths)
            )
            print(formatted_row)
    ...
    try:
        ...
        download_process = subprocess.Popen(
            ["wget", "-q", "-O", "-", snapshot_url], stdout=subprocess.PIPE
        )
        lz4_process = subprocess.Popen(
            ["lz4", "-d"], stdin=download_process.stdout, stdout=subprocess.PIPE
        )
        tar_process = subprocess.Popen(
            ["tar", "-C", osmosis_home, "-xf", "-"],
            stdin=lz4_process.stdout,
            stdout=subprocess.PIPE,
        )
...
"""
    ...
    if (
        operating_system in COSMOVISOR_URL
        and architecture in COSMOVISOR_URL[operating_system]
    ):
    ...
    env = {"DAEMON_NAME": "osmosisd", "DAEMON_HOME": osmosis_home}

    try:
        subprocess.run(
            ["/usr/local/bin/cosmovisor", "init", "/usr/local/bin/osmosisd"],
            check=True,
            env=env,
        )
    except subprocess.CalledProcessError:
